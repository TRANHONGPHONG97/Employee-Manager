...
function Location() {
  ...
  useEffect(() => {
    try {
      async function getData() {
        let resProvinces = await axios.get(
          "https://vapi.vnappmob.com/api/province/"
        );
        let resDistricts = await axios.get(
          `https://vapi.vnappmob.com/api/province/district/${province_id}`
        );
        let resWards = await axios.get(
  ...
  const handleChange = async (e) => {
    if (e.target.name === "province_id") {
      ...
      let resDistricts = await axios.get(
        `https://vapi.vnappmob.com/api/province/district/${province_value}`
      );
